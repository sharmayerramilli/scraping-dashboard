...
import io
from app.database import jobs_collection, results_collection, failed_urls_collection, sync_jobs_collection, sync_results_collection, sync_failed_urls_collection
...
router = APIRouter(prefix="/api/zomato", tags=["zomato"])

@router.post("/upload")
async def upload_csv(file: UploadFile = File(...)):
    """Upload CSV file with restaurant IDs - Handles large files"""
    try:
        # Read CSV
        ...
        logger.info(f"Uploading file: {file.filename}, Size: {file_size_mb:.2f} MB")
        ...
        # Clean and process IDs
        df = df.dropna(subset=['res_id'])  # Remove empty rows
        ...
        logger.info(f"✅ Created job {job_id} with {len(unique_res_ids)} unique URLs (removed {len(res_ids) - len(unique_res_ids)} duplicates)")
...
def run_scraping_job(job_id: str, res_ids: list):
    """Background task to run scraping"""
    try:
        logger.info(f"🚀 Starting scraping job {job_id} with {len(res_ids)} URLs")
        scraper = ZomatoScraper(job_id, num_workers=3)
...
async def start_scraping_with_ids(data: dict, background_tasks: BackgroundTasks):
    """Start scraping with provided IDs"""
    try:
        ...
        background_tasks.add_task(run_scraping_job, job_id, res_ids)
        
        logger.info(f"✅ Started scraping job {job_id} with {len(res_ids)} URLs")
        
        return {
            "success": True,
            "message": f"Scraping started for {len(res_ids)} URLs",
            "job_id": job_id
        }
...
async def get_status(job_id: str):
    """Get scraping job status with real-time counts"""
    try:
        job = await jobs_collection.find_one({"job_id": job_id})
...
async def download_results(job_id: str):
    """Download results as CSV with all extracted data"""
    try:
        ...
        logger.info(f"📥 Downloaded {len(results)} results for job {job_id}")
        
        return StreamingResponse(
            io.BytesIO(output.getvalue().encode()),
            media_type="text/csv",
            headers={"Content-Disposition": f"attachment; filename=zomato_results_{timestamp}.csv"}
        )
...
async def get_failed_urls(job_id: str):
    """Get failed URLs for a job with details"""
    try:
        failed = []
...
async def retry_failed_urls(job_id: str, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        # Get successful IDs
        successful_ids = set()
        async for doc in results_collection.find({"job_id": job_id}):
            successful_ids.add(doc["res_id"])
        
        # Get failed URLs that haven't succeeded
        failed = []
        async for doc in failed_urls_collection.find({"job_id": job_id}):
        ...
        logger.info(f"🔄 Retrying {len(failed)} failed URLs for job {job_id}")
        ...
        # Delete old failed records for these IDs
        ...
        background_tasks.add_task(run_scraping_job, job_id, failed)
